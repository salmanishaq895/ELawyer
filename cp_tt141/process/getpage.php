...
$sql = "SELECT * from `tt_advise` WHERE `seo_page` like '%".$q."%' order by `seo_page` DESC LIMIT 0,9";
...
if( mysql_num_rows($dataCity ) == 0 ) return;
	...
	{
		array_push($result, array("id"=>$cityRow['seo_page']  , "label"=>$cityRow['seo_page'], "value" => strip_tags($cityRow['seo_page'])));
		if (count($result) > 11)
		break;
	}
