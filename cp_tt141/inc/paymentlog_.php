...
<center><input type="submit" class="button" name="doSearch" id="doSearch" value="Filter Listing"   /></center> </form>			
	...
	$sql = "SELECT * FROM `tt_payment_history` $qryString $sortyby ";  
	$sqlcount = "SELECT count(locationid) FROM `tt_payment_history` $qryString"; 
	     ...
	     <?php } ?>	
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				  ...
				  <tr>
					...
					<td width="25"><strong>Trans.ID</strong></td>
				  ...
				  <?php
						 ...
						 while( $row = @mysql_fetch_array($result) )
						 {
							...
							?>
							   <tr>
								<td><?php echo ++$i;?> </td>
								<td><?php echo stripslashes($items['transaction_id']);?> </td>
								<td><?php if($items['status']==1) echo 'Active'; else echo 'Pending';?> </td>
								<td><?php echo $items['btype'];?> </td>
								<td><?php if($items['ltype']==1) echo 'SL'; else echo 'AL';?> </td>
								<td><?php echo $items['phone'];?> </td>
								<td><?php echo $items['city'].', '.$items['state'].', '.$items['zip'];?> </td>	                          
								<td><?php echo date ('Y-m-d',strtotime( $items['dated']) );?> </td>	
								<td>
                                    <!--<img src="images/details.gif"  style="cursor:pointer;" title="Reviews"   alt="Edit "   onClick="window.location='home.php?p=businessreviews&bId=<?php echo $items["locationid"];?>'"  />-->
