...
if ( isset ($_POST['doSearch'] ) || isset ($_POST['do_Search'] ) ) 
{
	...
	header("location:home.php?p=artical");exit;
}
$qryString = " where 1 ";
$qryString .= " AND advise_id <> 0 ";
...
//-------------------------------------------------------------------------
	...
	if( isset ($_GET['cid'] ) and  trim( $_GET['cid']) != '') {
			$cid = $_GET['cid'];
			$qryString = "where  advise_id = '".$cid."' " ;
		}
	
	$sortyby = '  order by artical_title';
...
//exit; 
 $sql = "select * from tt_artical $qryString $sortyby";

	$sqlcount = "SELECT count(artical_id) FROM `tt_artical` $qryString $sortyby"; 
?>
<h3><a href="#">Home</a> &raquo; <a href="#" class="active">Article management</a></h3>
<div class="content-box">
	<div class="content-box-header">
			<h3>Article management</h3>
			<div style="float:right;"> <h3><a href="<?php echo $ruadmin;?>home.php?p=new_advise_and_tip_artical" style=" margin-left:350px; font-size:12px; font-weight:bold;" > Add New Article </a></h3></div>
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				   <tr>
					...
					<?php 
						...
						echo "Article count: ".$rowcounts[0];
					?>
					 <div style="float:right; margin-right:5px;">
	 
     
     Sort By: <select name="cid" id="cid"  onChange="{window.location='home.php?p=artical&cid=' + this.value + '&sortby='+document.getElementById('cid').value}" >
				  ...
				  <?php 
						 ...
						 while( $row = @mysql_fetch_array($result) )
						 {
							...
							?>
							  <tr>
								<td><?php echo ++$i;?> </td>
								<td><?php echo stripslashes($items['artical_title']);?> </td>
							<!--	<td> <?php echo $items['seo_page'];?></td>
								...
								<?php
								$qry_category   = "select * from tt_advise where advise_id = '".$items['advise_id']."'";
								...
								<td>
						<img src="images/edit.gif"  style="cursor:pointer;" title="Edit "   alt="Edit "   onClick="window.location='home.php?p=edit_artical&artical_id=<?php echo $items["artical_id"];?>'"  />&nbsp;&nbsp;
						<img src="images/dlt.gif"  style="cursor:pointer;" title="Delete " alt="Delete " onClick="if(confirm('Are sure you want to delete')){ window.location='<?php echo $ruadmin ;?>process/process_artical.php?action=d&artical_id=<?php echo $items["artical_id"];?>'}"  />								</td>
							  </tr>	
