...
{
	$qrywhere .=  " and tt_payment_history.payment_status like '".$_SESSION['status']."' ";
}

if ( !isset($_SESSION['bl_SortBy']))
{
	$_SESSION['bl_SortBy'] = 'tt_payment_history.userId';
}
			...
			<tr>
				...
				<td>
					<select name="SortBy" onchange="document.getElementById(\'sText\').focus()">
						<option value="tt_user.firstname asc" <?php if ($_SESSION['bl_SortBy'] =='tt_user.firstname asc'){ echo 'selected="selected" ';}?>>First Name Asc</option>
						<option value="tt_user.firstname desc" <?php if ($_SESSION['bl_SortBy'] =='tt_user.firstname desc'){ echo 'selected="selected" ';}?>>First Name Desc</option>
						<option value="tt_user.lastname asc" <?php if ($_SESSION['bl_SortBy'] =='tt_user.lastname asc'){ echo 'selected="selected" ';}?>>Last Name Asc</option>
						<option value="tt_user.lastname desc" <?php if ($_SESSION['bl_SortBy'] =='tt_user.lastname desc'){ echo 'selected="selected" ';}?>>Last Name Desc</option>
						<option value="tt_payment_history.payment_date asc" <?php if ($_SESSION['bl_SortBy'] =='tt_payment_history.payment_date asc'){ echo 'selected="selected" ';}?>>Payment Date Asc</option>
						<option value="tt_payment_history.payment_date desc" <?php if ($_SESSION['bl_SortBy'] =='tt_payment_history.payment_date desc'){ echo 'selected="selected" ';}?>>Payment Date Desc</option>
					</select>
				</td>
				<td>
...
/*	$qry_merchant= "select  a.pid,a.transaction_id,a.payment_amount,a.payment_date,a.dateexpiry,a.payment_status,u.firstname,u.lastname from tt_payment_history as a LEFT JOIN tt_user as u on a.userId=u.userId $qrywhere $sortyby "; //$qrywhere $sortyby
	...
	$qry_merchant= "SELECT 
				  `tt_user`.firstname,
				  `tt_user`.lastname,
				  `tt_payment_history`.transaction_id,
				  `tt_payment_history`.payment_amount,
				  `tt_payment_history`.payment_date,
				  `tt_payment_history`.payment_status,
				  `tt_payment_history`.locationid,
				  `tt_payment_history`.userId,
				  `tt_payment_history`.pid,
				  `tt_business`.name
				FROM
				  `tt_payment_history`
				  INNER JOIN `tt_user` ON (`tt_payment_history`.userId = `tt_user`.userId)
				  INNER JOIN `tt_business` ON (`tt_payment_history`.locationid = `tt_business`.locationid) $qrywhere "; //$qrywhere $sortyby
	
	 $qry_merchant_count= "SELECT  count(tt_payment_history.pid) as mcount,
					  `tt_user`.firstname,
					  `tt_user`.lastname,
					  `tt_payment_history`.transaction_id,
					  `tt_payment_history`.payment_amount,
					...
					FROM
					  `tt_payment_history`
					  INNER JOIN `tt_user` ON (`tt_payment_history`.userId = `tt_user`.userId)
					  INNER JOIN `tt_business` ON (`tt_payment_history`.locationid = `tt_business`.locationid) $qrywhere $sortyby  "; //$qrywhere $sortyby 	
