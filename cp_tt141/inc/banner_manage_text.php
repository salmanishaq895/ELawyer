...
{
	$_SESSION['banner_bizStatus']=$_POST['userStatus'];
	$_SESSION['banner_sType']=$_POST['sType'];
	$_SESSION['banner_sText']=trim($_POST['sText']);
	$_SESSION['banner_SortBy']=$_POST['SortBy'];
	$_SESSION['ltype'] = $_POST['ltype'];
	header("location:".$ruadmin."home.php?p=banner_manage_text");exit;
}
$qryString = " where 1 and banner_type ='text'  ";

if ( isset ($_GET['userId']) && ($_GET['userId']!='')){
	$qryString .= " and tt_banner.userId=".$_GET['userId'];
}

if ( !isset($_SESSION['banner_bizStatus']))
{
	$_SESSION['banner_bizStatus'] = 'n'; 
}

if($_SESSION['banner_bizStatus']=="n")
{
	$qryString .=  " ";
}
else
{
	$qryString .= " and  tt_banner.status = ".$_SESSION['banner_bizStatus']." ";	
}
...
if ( $_SESSION['banner_sText'] != '' )
{
	$qryString .= " and ".$_SESSION['banner_sType'] ." like '".$_SESSION['banner_sText']."%'";
}
	
if ( !isset($_SESSION['banner_SortBy']))
{
	$_SESSION['banner_SortBy'] = ' tt_banner.banner_id ';
}
...
$t.='
	...
	<tr>
		...
		<form method="post" action="">Status:&nbsp;<select name="userStatus"  onchange="document.getElementById(\'sText\').focus()">			
			<option';
			if ($_SESSION['banner_bizStatus'] =='n' )  $t.=' selected="selected" ';
			$t.=' value="n">All </option><option ';
			if ($_SESSION['banner_bizStatus'] =='1' )  $t.=' selected="selected" ';
			$t.=' value="1">Active</option>	<option ';
			if ($_SESSION['banner_bizStatus'] =='0' )  $t.=' selected="selected" ';
			$t.=' value="0">Block</option>
						
			</select>&nbsp;Search:&nbsp;<select name="sType" id="sType"   onchange="document.getElementById(\'sText\').focus()">			
			<option ';
			if ($_SESSION['banner_sType'] =='tt_user.firstname' )  $t.=' selected="selected" ';
			$t.='value="tt_user.firstname">Advertiser First Name</option>	
			<option ';
			if ($_SESSION['banner_sType'] =='tt_user.lastname' )  $t.=' selected="selected" ';
			$t.='value="tt_user.lastname">Advertiser Last Name</option>						

			<option ';
			if ($_SESSION['banner_sType'] =='tt_user.email' )  $t.=' selected="selected" ';
			$t.=' value="tt_user.email">Email</option>
			
			<option ';
			if ($_SESSION['banner_sType'] =='tt_banner.site_url' )  $t.=' selected="selected" ';
			$t.=' value="tt_banner.site_url">Site URL</option>
			
			<option ';
			if ($_SESSION['banner_sType'] =='tt_banner.text_title' )  $t.=' selected="selected" ';
			$t.=' value="tt_banner.text_title">Banner Title</option>
			
			<option ';
			if ($_SESSION['banner_sType'] =='tt_banner.text_description' )  $t.=' selected="selected" ';
			$t.=' value="tt_banner.text_description">Banner Description</option>
			
			</select>
			<input type="text" id="sText"  name="sText" class="text-input" value="'.$_SESSION['banner_sText'].'">
	...
	$sortyby = '  order by '.$_SESSION['banner_SortBy'];
	 $sql = "SELECT tt_user.firstname
					 ...
					 , tt_banner.text_title
					 , tt_banner.text_description
					 , tt_banner.site_url
				FROM
				  tt_user
	...
	unset($_SESSION['banner_bizStatus']);
	unset($_SESSION['ltype']);
	unset($_SESSION['banner_sText']);
	unset($_SESSION['banner_SortBy']);
	unset($_SESSION['banner_bizPackage']);	
?>
<h3><a href="#">Home</a> &raquo; <a href="#" class="active">Banner Text Management</a></h3>
<div class="content-box">
	<div class="content-box-header">
			<h3>Banner Text Management</h3>
		<div class="clear"></div>
	</div>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				 ...
				 <tr>
				  ...
				  <tr>
					...
					<td width="15%"><strong>Advertiser Name</strong></td>
						 ...
						 while( $row = @mysql_fetch_array($result) )
						 {
							...
							?>
							   <tr>
								...
								<td><?php echo $items['text_title'];?> </td>
								<td><?php echo $items['text_description'];?> </td>	                          
								<!--<td><?php echo date ('Y-m-d',strtotime( $items['dated']) );?> </td>-->
								<td>
									<select name="status" id="status" class="select1" style="width:100%" 
									onchange="showUser(this.value,'<?php echo $items['banner_id']; ?>','status','<?php echo stripslashes($items['text_title']);?>','image')"  >
