...
if ( isset ($_POST['doSearch'] ) ) 
{
	...
	header("location:".$ruadmin."home.php?p=emails_bulk_trader");exit;
}
...
{
	$qryString .= " and `tt_user`.status = 'Active' ";	
}
elseif($_SESSION['em_userStatus']=="Pending")
{
	$qryString .= " and `tt_user`.status = 'Pending' ";	
}
elseif($_SESSION['em_userStatus']=="Deactive")
{
	$qryString .= " and `tt_user`.status = 'Deactive' ";	
}
$innerJoin = '';

$innerJoin .= 'LEFT JOIN `tt_business` ON (`tt_user`.userId = `tt_business`.userId)';

if($_SESSION['Category']!=''){
	$innerJoin .= ' LEFT JOIN `tt_category` ON (`tt_business`.mcatid=`tt_category`.catid) ';
}
...
if( $_SESSION['Category']!='')
{
	$qryString .= " and mcatid 	 = '" . $_SESSION['Category'] . "'";
}
...
*/
	$c = '';
	$sql_category  = "select * from tt_category where p_catid='0' and cat_type='1' order by cat_name";
	  $result_category = mysql_query($sql_category);
	   $c .= 'Sort By Category<select name="Category">';
	   $c.='<option value="">All</option>';
	   while($row_category = mysql_fetch_array($result_category))
	   {
			$c .= '<option value="'.$row_category['catid'].'"';
			if($_SESSION['Category'] == $row_category['catid'])
				$c .= ' selected="selected" ';
			$c .= '> '.$row_category['cat_name'].'</option>';
	   }
	   $c .='</select>';
	//echo $c; exit;
...
$t.='
	...
	//$sql_sel_user = "SELECT tt_user.email, tt_user.firstname, tt_user.lastname, tt_user.zip, tt_user.city, tt_user.state FROM `tt_user` $innerJoin $qryString and tt_user.type = 't' $sortyby ";  
	
	$sql_sel_user = "SELECT `tt_user`.userId,`tt_user`.email, `tt_user`.firstname, `tt_user`.lastname, `tt_user`.zip, `tt_user`.city, `tt_user`.state,`tt_business`.name,`tt_business`.locationid,`tt_business`.mcatid,`tt_business`.userId
			FROM `tt_user` $innerJoin $qryString and `tt_user`.type='t'"; //exit;
  ...
  <?php } ?>	
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
		...
		<tr>
			<td>
			<input type="submit" class="button" name="SaveTextData_customer" value="Send Email Trader" /></td>
		</tr>
	</table>
