...
$t.='
	...
	//SELECT a.firstname,a.lastname,a.email,a.loginlogout,a.city,a.state,a.zip,a.dated,a.status,a.type,b.name,b.locationid FROM tt_business as b  INNER JOIN `tt_user` as a ON a.userId=b.userId $qryString $sortyby 
  	$sql = "SELECT `tt_user`.userId,`tt_user`.firstname,`tt_user`.lastname,`tt_user`.email,`tt_user`.type,`tt_user`.status,`tt_user`.dated,`tt_business`.name,`tt_business`.locationid,`tt_business`.city,`tt_business`.state,`tt_business`.zip
		FROM `tt_user` LEFT JOIN `tt_business` ON (`tt_user`.userId = `tt_business`.userId)  $qryString $sortyby ";  
	
	$sqldownload = "SELECT `tt_user`.userId,`tt_user`.firstname,`tt_user`.lastname,`tt_user`.email,`tt_user`.type,`tt_user`.status,`tt_user`.dated,`tt_business`.name,`tt_business`.locationid,`tt_business`.city,`tt_business`.state,`tt_business`.zip
		FROM `tt_user` LEFT JOIN `tt_business` ON (`tt_user`.userId = `tt_business`.userId)  $qryString $sortyby ";  
...
<h3><a href="#">Home</a> &raquo; <a href="#" class="active">Trade User Management</a></h3>
<div class="content-box">
	<div class="content-box-header">
			<h3> Trade User Management</h3>
			<div style="float:right;"> <h3><a href="<?php echo $ruadmin;?>home.php?p=user_add_trade" style=" margin-left:350px; font-size:12px; font-weight:bold;" > Add New Trade User </a></h3></div>
		<div class="clear"></div>
	</div>
						...
						?>
						  <tr>
							...
							<td><?php if($items['name']!=''){ ?>
							 <a href="<?php echo $ruadmin; ?>home.php?p=editbusiness&bId=<?php echo $items['locationid'];?> ">
							<?php echo $items['name']; ?>
							</a>
							<?php }else{ }?> </td>
							...
							<img src="images/mail_send.png"  style="cursor:pointer;" title="email "   alt="email "   onClick="window.location='<?php echo $ruadmin; ?>process/process_user.php?action=e&userId=<?php echo $items["userId"];?>&email=<?php echo $items["email"];?>'"  />&nbsp;&nbsp;
							<?php } ?>
							<img src="images/edit.gif"  style="cursor:pointer;" title="Edit "   alt="Edit "   onClick="window.location='home.php?p=user_trade_edit&userId=<?php echo $items["userId"];?>'"  />&nbsp;&nbsp;
							...
							<?php if($items['name']!=''){ ?>
							 <a href="<?php echo $ruadmin; ?>home.php?p=editbusiness&bId=<?php echo $items['locationid'];?> ">
							<img src="images/edit_f2.png"  style="cursor:pointer;" title="Edit Business"   alt="Edit Business" width="18" height="18"/>
							</a>
							<?php }else{ }?> 							
							</td>
						  </tr>	
