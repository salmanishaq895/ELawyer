...
$job_name = $job_data['title']." ,".$job_data['location'];

$update_msg_count_qry = "update tt_messages set `to_viewed` = '1'  where `to` = '".$trader_id."' and jobId = '".$job_id."'  ";
$exe_msg_count_qry = @mysql_query($update_msg_count_qry);


$messages_qry = "SELECT * FROM tt_messages where 	jobId = '".$job_id."' and (`from` = '".$trader_id."' or `to` = '".$trader_id."') ORDER BY  created desc "; //exit;
 ...
 <div class="main_quote_bar_b">
  <div class="map_page_right_bar">
    <div class="brued_crum_bar brued_crum_bar_c">
      <div class="listing_page_brued_crum"> <span class="brurd_curm_inner"><a href="<?php echo $ru;?>" style="text-decoration:none; color:#999999;">Home</a> > <a href="<?php echo $ru;?>member/applied_job/" style="text-decoration:none; color:#999999;">Applied Jobs</a> > <a href="<?php echo $ru;?>member/job_messages/<?php echo $job_id ?>" style="text-decoration:none; color:#999999;"> <span class="change">Messages</span></a></span> </div>
    </div>
    <div class="profile_page_left">
      <div class="listing_wrapper_left"> <span style="width:97%;">Send Message To Customer ( <?php echo $job_name?> ) </span> </div>
      <form method="post" action="<?php echo $ru; ?>process/process_msg.php">
	  <input type="hidden" name="from_id" value="<?php echo  $trader_id; ?>" />
    ...
    <div onMouseOut="hideshowOut(<?php echo $qry_row['quotes_id'];?>)" onMouseOver="hideshowOver(<?php echo $qry_row['quotes_id'];?>)" class="listing_outer_tabe">
      <div id="buss_cover_div_<?php echo $qry_row['quotes_id'];?>" style="height:auto;" class="listing_inner_top_bar">
        <div class="listing_inner_top_bar_a" style="margin:0px;"> 
			<span>
        ...
        <p style="margin-left:10px; font-weight:bold;"><a href="javascript:" style="color:#5F5F5F; text-decoration:none;"><?php echo ucfirst( $qry_row['title'] );?></a></p>
        <p class="listing_inner_top_p" style="margin-left:10px;"><img src="<?php echo $ru; ?>images/listing_home_icon.png">
		<a href="javascript:" style="color:#5F5F5F; text-decoration:none;"><?php echo stripslashes(ucfirst( $qry_row['message'] ));?></a></p>
      </div>
	...
	<?php } } unset($_SESSION['get_quote_err']);?>
