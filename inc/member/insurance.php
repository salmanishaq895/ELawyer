...
<div class="map_page_right_bar listing_right_bar">
  <div class="brued_crum_bar brued_crum_bar_c">
    <div class="listing_page_brued_crum"> <span class="brurd_curm_inner"><a href="<?php echo $ru;?>" style="text-decoration:none; color:#999999;" >Home</a> > <a href="<?php echo $ru;?>member/statistics" style="text-decoration:none; color:#999999;"> Account Panel</a> ><a href="<?php echo $ru;?>member/trade-profile" style="text-decoration:none; color:#999999;"> Trader Profile ></a>  <a href="<?php echo $ru;?>member/insurance/<?php if(isset($_GET['o'])) echo $_GET['o'];?>" style="text-decoration:none; color:#999999;" > <span class="change"> Insurnace </span> </a></span></div>
  </div>
	<?php include('notice.php'); ?>
    <div class="cpanel_right_bar">
      <div class="company_detail_description">
	  	<span class="company_detail_span">Manage Insurance</span>
		<?php if(isset($_SESSION['insurance_msg'])){?>
			<div style="float:left; width:500px; margin-left:50px; color:#FF0000;"><?php echo $_SESSION['insurance_msg']; ?></div>
		<?php } unset($_SESSION['insurance_msg']); ?>
	  	<?php 
		$res_key = mysql_query("SELECT * FROM `tt_business_insurance` WHERE `userId` = '$userId' "); 
		$count = 0;
		$keyId = 0;
		if(isset($_GET['o']))
			$keyId = (int)$_GET['o'];
		...
		<?php
			...
			<?php 
			if( $keyId==$row_key['id'] ){
				$qry_key = mysql_query("SELECT * FROM `tt_business_insurance` WHERE `id` = '$keyId'");
				$_SESSION['insurance'] = mysql_fetch_array($qry_key);
			?>
          ...
          <div class="input_flied">
            ...
            <input name="title" id="title" type="text" <?php if(isset($_SESSION['insurance_err']['title'])) echo 'style="border:1px solid #FF0000;"';?> class="text-input" value="<?php echo stripslashes($_SESSION['insurance']['title']); ?>" onkeyup="return titlecount(40)" />
			<samp style="margin:0 0 0 130px; font-size:12px; font-family:'MyriadProCondensed';" id="colorr">
				...
				echo (40 - count(explode(' ',$_SESSION['skill']['title'])));
				 ?></samp> words</samp><br />
		  ...
		  <?php if(!empty($_SESSION['insurance']['img']) and file_exists($rootpath . "media/insurance/" . $userId . "/thumb/".$_SESSION['insurance']['img']) ){ ?>
          <div class="input_flied">
            <div>&nbsp;</div>
            <div class="upload_image_bar" style="text-align:left; width:85px;">
			
		<!--	<a href="<?php echo $ru . 'media/insurance/'.$_SESSION['insurance']['userId'].'/'.$_SESSION['insurance']['img'];?>" target="_blank">	<img src="<?php echo $ru.'images/link.jpg';?>" style="max-width:43px;" /></a>
			</div>
			</div>
			-->
              <img src="<?php echo $ru."media/insurance/" . $userId . "/thumb/".$_SESSION['insurance']['img'];?>" title="thumb" style="margin:0;" alt="" />
              <br/>
              Image </div>
          </div>
		  <?php 
		  }else if(!empty($_SESSION['insurance']['img']) and file_exists($rootpath . "media/qualification/" . $userId . "/".$_SESSION['insurance']['img'])){
		  ?>
		  <div class="input_flied">
            <div>&nbsp;</div>
            <div class="upload_image_bar" style="text-align:left; width:85px;">
          ...
          <div class="input_flied">
            ...
            <input type="hidden" name="oldImg" value="<?php echo $_SESSION['insurance']['img'];?>" />
          </div>
          <div class="input_flied">
            <div>Description <samp>* </samp></div>
            <textarea name="shortdescription" id="shortdescription"  rows="4" cols="72" type="text" <?php if(isset($_SESSION['insurance_err']['shortdescription'])) echo 'style="border:1px solid #FF0000;"';?>  onkeyup="return textCounter(450)" ><?php echo stripslashes($_SESSION['insurance']['shortdescription']); ?></textarea>
			<samp style="margin:0 0 0 130px; font-size:12px; font-family:'MyriadProCondensed';" id="colorr">
		  ...
		  <div class="profile_botton_outer" style="margin-bottom:10px; float:left;">
            <div class="contact_us_gray_botton_inner cpanel_right_botton_button">
              ...
              <input type="submit" value="&nbsp;&nbsp;&nbsp;Update&nbsp;&nbsp;&nbsp;" name="addInsurance" id="addInsurance" class="inner_gray_botton" />
			  <input type="button" style="margin-left:10px;" onclick="window.location='<?php echo $ru; ?>member/insurance/';" value="&nbsp;&nbsp;&nbsp;Cancel&nbsp;&nbsp;&nbsp;" class="inner_gray_botton" />
            </div>
          </div>
		  </form>
		  <?php
			...
			<div class="title"><?php echo $row_key['title']; ?></div>
			 <div class="img"><?php 
			  if(file_exists($rootpath.'media/insurance/'.$userId.'/thumb/'.$row_key['img']) and !empty($row_key['img'])){?>
			  	
				<!--<a href="<?php echo $ru . 'media/insurance/'.$userId.'/'.$row_key['img'];?>" target="_blank">	<img src="<?php echo $ru.'images/link.jpg';?>" style="max-width:43px;" /></a>
			
			</div>
			</div>-->
			
				
				<img src="<?php echo $ru.'media/insurance/'.$userId.'/thumb/'.$row_key['img'];?>" width="57" />
			  <?php 
		  ...
		  <?php
			...
			<div class="desc"><?php echo $row_key['shortdescription']; ?></div>
			 <div class="action">
			  	<a href="<?php echo $ru; ?>member/insurance/<?php echo $row_key['id']; ?>"><img src="<?php echo $ru; ?>images/edit.gif" /></a>
				<a onclick="return confirm('Are you sure you want to delte this service?');" href="<?php echo $ru; ?>process/process_business.php?action=dlt_insurance&keyid=<?php echo $row_key['id']; ?>"><img src="<?php echo $ru; ?>images/drop.gif" /></a>
			  </div>
		 <?php 
		  $count++;
		}
	  ...
	  <form name="form1" id="form1" method="post" action="<?php echo $ru;?>process/process_business.php" enctype="multipart/form-data" >
          <div class="input_flied">
		  <h2>Add new Insurnace</h2>
		  </div>
          <div class="input_flied">
            <div>Title <samp>* </samp></div>
            <input name="title" id="title" type="text" class="text-input" <?php if(isset($_SESSION['insurance_err']['title'])) echo 'style="border:1px solid #FF0000;"';?> value="<?php echo stripslashes($_SESSION['insurance']['title']); ?>" onkeyup="return titlecount(40)" />
			<samp style="margin:0 0 0 130px; font-size:12px; font-family:'MyriadProCondensed';" id="colorr">
				You can enter upto <samp id="titleString" style="font-size:12px;font-family:'MyriadProCondensed';"><?php
				echo (40 - count(explode(' ',$_SESSION['skill']['title'])));
				 ?></samp> words</samp><br />
          ...
          <div class="input_flied">
            <div>Image/ file </div>
            <input name="img" id="img" type="file" value="<?php echo $_SESSION['insurance']['img'];?>" style="width:auto; font-size:12px;" />
            &nbsp;&nbsp;&nbsp;(image size 170 X 115 )
          </div>
          <div class="input_flied">
            <div>Description <samp>* </samp></div>
            <textarea name="shortdescription" id="shortdescription" <?php if(isset($_SESSION['insurance_err']['shortdescription'])) echo 'style="border:1px solid #FF0000;"';?> rows="4" cols="72" type="text" onkeyup="return textCounter(450)" ><?php echo stripslashes($_SESSION['insurance']['shortdescription']); ?></textarea>
			<samp style="margin:0 0 0 130px; font-size:12px; font-family:'MyriadProCondensed';" id="colorr">
				You can enter upto <samp id="inputString" style="font-size:12px;font-family:'MyriadProCondensed';"><?php
				echo (450 - count(explode(' ',$_SESSION['skill']['shortdescription'])));
				 ?></samp> words</samp> <br />
		  ...
		  <div class="profile_botton_outer">
            <div class="contact_us_gray_botton_inner cpanel_right_botton_button">
			  <input type="submit" value="&nbsp;&nbsp;&nbsp;Submit&nbsp;&nbsp;&nbsp;" name="addInsurance" id="addInsurance" class="inner_gray_botton" />
            </div>
          </div>
		  </form>
		<?php 
