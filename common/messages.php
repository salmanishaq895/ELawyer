...
class cpm {
    ...
    function getmessages($type=0) {
        ...
        switch($type) {
            case "0": $sql = "SELECT * FROM tt_messages WHERE `to` = '".$this->userid."' && `to_viewed` = '0' && `to_deleted` = '0' ORDER BY `created` DESC"; break; // New messages
            case "1": $sql = "SELECT * FROM tt_messages WHERE `to` = '".$this->userid."' && `to_viewed` = '1' && `to_deleted` = '0' ORDER BY `to_vdate` DESC"; break; // Read messages
            case "2": $sql = "SELECT * FROM tt_messages WHERE `from` = '".$this->userid."' ORDER BY `created` DESC"; break; // Send messages
            case "3": $sql = "SELECT * FROM tt_messages WHERE `to` = '".$this->userid."' && `to_deleted` = '1' ORDER BY `to_ddate` DESC"; break; // Deleted messages
            default: $sql = "SELECT * FROM tt_messages WHERE `to` = '".$this->userid."' && `to_viewed` = '0' ORDER BY `created` DESC"; break; // New messages
        }
    ...
    // Fetch the <span class="IL_AD" id="IL_AD9">username</span> from a userid, I made this function because I don't know how you did build your usersystem, that's why I also didn't use left join... this way you can easily edit it
    function getusername($userid) {
        $sql = "SELECT `firstname`,`lastname` FROM `tt_user` WHERE `userId` = '".$userid."' LIMIT 1";
    ...
    function getmessage($message) {
        $sql = "SELECT * FROM tt_messages WHERE `id` = '".$message."' && (`from` = '".$this->userid."' || `to` = '".$this->userid."') LIMIT 1";
        $result = mysql_query($sql);
        if(mysql_num_rows($result)) {
            // reset the array
            $this->messages = array();
			...
			$this->messages[0]['msg_thread'] = $row['msg_thread'];
			$this->messages[0]['jobId'] = $row['jobId'];
			$this->messages[0]['message_frm'] = $row['message_frm'];
   ...
   // We need the userid for pms, but we only let users input usernames, so we need to get the userid of the username :)
    ...
    function viewed($message) {
        $sql = "UPDATE tt_messages SET `to_viewed` = '1', `to_vdate` = NOW() WHERE `id` = '".$message."' LIMIT 1";
        return (@mysql_query($sql)) ? true:false;
    }
    
    // Flag a message as deleted
    function deleted($message) {
        $sql = "UPDATE tt_messages SET `to_deleted` = '1', `to_ddate` = NOW() WHERE `id` = '".$message."' LIMIT 1";
        return (@mysql_query($sql)) ? true:false;
    }
    
    // Add a new personal message
    function sendmessage($to,$jobId,$title,$message,$threadId,$message_frm = 'Trader') {
        //$to = $this->getuserid($to);
        $sql = "INSERT INTO tt_messages SET `to` = '" . $to . "', `from` = '" . $this->userid . "', `jobId` = '" . $jobId . "', 
		`title` = '" . $title . "', `message` = '" . addslashes($message) . "', `msg_thread` = '$threadId',
		message_frm = '$message_frm', `created` = NOW()";
		return (@mysql_query($sql)) ? true:false;
    }
    
    // Render the text (in here you can easily add bbcode <span class="IL_AD" id="IL_AD3">for example</span>)
