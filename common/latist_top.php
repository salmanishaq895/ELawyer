...
if($page == 'map_location' || $page == 'listings'){
	...
	$orderby = "";
	$radd    = "";
	$qrystring .= " WHERE b.status = 1 AND b.name <> '' ";
	
	if($_GET['s'] == 'category'){
		$arr = explode('_',$_REQUEST['o']);
		$catId = $arr[0];
		$qrystring .= " AND ( b.`mcatid` = '$catId' or b.`scatids` like ',%$catId%,')";
	}else{
		$keyword = str_replace("_"," ",$_GET['s']);
		$cityZip =  str_replace("_"," ",$_GET['o']);
		if(is_numeric($_GET['p']) and !empty($_GET['p']) and $cityZip != '' and $cityZip != 'all' ){
			$postCodeAds = urlencode(stripslashes($cityZip).", UK");
			$geocode = file_get_contents('http://maps.google.com/maps/api/geocode/json?address='.$postCodeAds.'&sensor=false') or die('ZAHIDFDFD');
			$output = json_decode($geocode);
			$lat = $output->results[0]->geometry->location->lat;
			$long = $output->results[0]->geometry->location->lng;
			if( !empty($lat) and !empty($long) ){
				$qry_lat .= " , ( 6371 * acos( cos( radians( {$lat} ) ) * cos( radians( b.latitude ) ) * cos( radians( b.longitude ) - radians( {$long} ) ) 
					+ sin( radians( {$lat} ) ) * sin( radians( b.latitude ) ) ) ) as radius ";
					$radd  .= "HAVING radius <= '".$_GET['p']."'";
			}else{
				$qrystring .= " AND ( b.city =  '$cityZip' OR b.zip = '$cityZip' OR b.address2 = '$cityZip' )";
			}
		}else if($cityZip!='' && $cityZip != 'all'){
		...
		if($keyword!='' && $keyword!= 'all'){
			$qrystring .= " AND b.name LIKE '%$keyword%' OR ( b.industry LIKE '%$keyword%' OR b.subcat LIKE '%$keyword%' OR b.keywords LIKE '%$keyword%') ";
		}
	}
	...
	//$radd .=" AND ( b.city =  '$cityZip' OR b.zip = '$cityZip' OR b.address2 = '$cityZip' )";
	
	
	
	if($_GET['p']=="review"){
				$orderby .= " ORDER BY b.review_count DESC ";
	}elseif($_GET['p']=="location"){
		$lat = $_SESSION['TTLOGINDATA']['lati'];
		$long = $_SESSION['TTLOGINDATA']['long'];
		if($lat!='' && $long!=''){
			$qry_lat .= " , ( 6371 * acos( cos( radians( {$lat} ) ) * cos( radians( b.latitude ) ) * cos( radians( b.longitude ) - radians( {$long} ) ) 
					+ sin( radians( {$lat} ) ) * sin( radians( b.latitude ) ) ) ) as radius";
			$orderby .= " ORDER BY radius DESC ";
		}
		//echo $qry_lat;exit;
	}else if($_GET['p']=="photo"){
		$orderby .= " ORDER BY b.photoCount DESC ";
	}else{
		$orderby .= " ORDER BY b.name ASC ";
	}
	
	$sqlcount = "SELECT b.locationid $qry_lat FROM tt_business as b $qrystring $radd";
	//echo  $sqlcount; exit;
	$qrycounts = mysql_query($sqlcount);
	$total_pages = mysql_num_rows($qrycounts);
	//$total_pages = $rowcounts[0];
	//echo $total_pages; exit;
   $business_query = "SELECT b.* $qry_lat FROM tt_business as b  $qrystring $radd $orderby";
	/*echo $business_query."<br/>";
	echo $sqlcount;
	//echo $total_pages; 
	exit;*/ 
	///////// //////////////// //////////////////////////////////////////////////////////////
...
$qrySEO="select * from `tt_seo` where page like '". $page ."'"; //exit;
...
if($rsSEO){
	...
	if($page=='listings'){
		...
		if($page=='advice'){
			if($s!='' && $o==''){
				$s = $_GET['s'];
				$s_cms = "SELECT * FROM `tt_advise` WHERE `seo_page` = '$s' " ;
				$ss_cms = $db->get_row($s_cms, ARRAY_A); 	
				if($ss_cms){
					$pageTitle = $ss_cms['advise_title'];
					$seoTitle =$ss_cms['meta_title'];
					$seoKeywords = $ss_cms['meta_keyword'];
					$seoDescription = $ss_cms['meta_description'];		
				}
			}else if($s!='' && $o!='' && $p==''){
				$seo_page = $_GET['o'];
				$o_cms = "SELECT * FROM `tt_advise` WHERE `seo_page` = '$seo_page' " ;
				$os_cms = $db->get_row($o_cms, ARRAY_A); 	
				if($os_cms){
					$pageTitle = $os_cms['advise_title'];
					$seoTitle =$os_cms['meta_title'];
					$seoKeywords = $os_cms['meta_keyword'];
					$seoDescription = $os_cms['meta_description'];		
				}
			}else if($s!='' && $o!='' && $p!=''){
				$p_page = $_GET['p'];
				$p_cms = "SELECT * FROM `tt_artical` WHERE `seo_page` = '$p_page' " ;
				$ps_cms = $db->get_row($p_cms, ARRAY_A); 	
				if($ps_cms){
					$pageTitle = $ps_cms['artical_title'];
					$seoTitle =$ps_cms['meta_title'];
					$seoKeywords = $ps_cms['meta_keyword'];
					$seoDescription = $ps_cms['meta_description'];		
				}
			}else{
				$catgory = ucfirst(str_replace("-"," ",$s));
				$post = ucfirst(str_replace("-"," ",$o));
									
				$pageTitle = str_replace('[category]',$catgory,$pageTitle);
				$seoTitle = str_replace('[category]',$catgory,$seoTitle);
				$seoKeywords = str_replace('[category]',$catgory,$seoKeywords);
				$seoDescription = str_replace('[category]',$catgory,$seoDescription);		
				
				$pageTitle = str_replace('[post_title]',$post,$pageTitle);
				$seoTitle = str_replace('[post_title]',$post,$seoTitle);
				$seoKeywords = str_replace('[post_title]',$post,$seoKeywords);
				$seoDescription = str_replace('[post_title]',$post,$seoDescription);
			}
		}
	}
	...
	function initialize() {
		...
		// echo "<pre>";print_r($rs_business); exit;
		 ...
		 foreach($rs_business as $res){
		  ...
		  //echo $image; exit;
		?> 
		 { lat: <?php echo $res['latitude'];?>, lng:<?php echo $res['longitude'];?>, name: "<?php echo $res['name'];?>", html: '<table cellpadding="0" cellspacing="0" border="0" ><tr><td colspan="3"><a href="<?php echo $ru;?>profile/<?php echo $res['locationid']."_".encodeURL(stripslashes($res['name']))."/"; if($res['industry']!=''){echo encodeURL(stripslashes($res['industry']))."/";}    if($res['city']!=''){ echo encodeURL(stripslashes($res['city']))."/";} if($res['state']!=''){echo encodeURL(stripslashes($res['state']))."/";} echo $res['zip'] ;?>" style="color:#3f3f3f; " ><?php echo $res['name'];?></a></td></tr><tr><td colspan="3"><a href="<?php echo $ru;?>profile/<?php echo $res['locationid']."_".encodeURL(stripslashes($res['name']))."/"; if($res['industry']!=''){echo encodeURL(stripslashes($res['industry']))."/";}    if($res['city']!=''){ echo encodeURL(stripslashes($res['city']))."/";} if($res['state']!=''){echo encodeURL(stripslashes($res['state']))."/";} echo $res['zip'] ;?>" style="color:#3f3f3f; " ><img src="<?php echo $ru."media/buisness_images/".$res['locationid']."/logo/thumb/".$res['logo'];?>" title="<?php echo $res['name'];?>" alt="<?php echo $res['name'];?>" width="150" hight="10" border="0" /></a></td></tr><tr><td>Address</td><td class="2"><?php echo $res['address'];?></td></tr><tr><td >Phone No:</td><td colspan="2"><?php echo $res['phone'];?></td></tr></table>', icon: '<?php echo $image; ?>' },<?php //if($i == 0) echo ',';?>
		 <?php 
			$i++;
			}
		 }
	...
	function initialize() {
		...
		// echo "<pre>";print_r($rs_business); exit;
		 ...
		 foreach($rs_business as $res){
		  ...
		  //echo $image; exit;
		?> 
		 { lat: <?php echo $res['latitude'];?>, lng:<?php echo $res['longitude'];?>, name: "<?php echo $res['name'];?>", html: '<table cellpadding="0" cellspacing="0" border="0" ><tr><td colspan="3"><a href="<?php echo $ru;?>profile/<?php echo $res['locationid']."_".encodeURL(stripslashes($res['name']))."/"; if($res['industry']!=''){echo encodeURL(stripslashes($res['industry']))."/";}    if($res['city']!=''){ echo encodeURL(stripslashes($res['city']))."/";} if($res['state']!=''){echo encodeURL(stripslashes($row_business['state']))."/";} echo $row_business['zip'] ;?>" style="color:#3f3f3f; " ><?php echo $res['name'];?></a></td></tr><tr><td colspan="3"><a href="<?php echo $ru;?>profile/<?php echo $res['locationid']."_".encodeURL(stripslashes($res['name']))."/"; if($res['industry']!=''){echo encodeURL(stripslashes($res['industry']))."/";}    if($res['city']!=''){ echo encodeURL(stripslashes($res['city']))."/";} if($res['state']!=''){echo encodeURL(stripslashes($res['state']))."/";} echo $res['zip'] ;?>" style="color:#3f3f3f; " ><img src="<?php echo $ru."media/buisness_images/".$res['locationid']."/logo/thumb/".$res['logo'];?>" title="<?php echo $res['name'];?>" alt="<?php echo $res['name'];?>" width="150" hight="10" border="0" /></a></td></tr><tr><td>Address</td><td class="2"><?php echo $res['address'];?></td></tr><tr><td >Phone No:</td><td colspan="2"><?php echo $res['phone'];?></td></tr></table>', icon: '<?php echo $image; ?>' },<?php //if($i == 0) echo ',';?>
		 <?php 
			$i++;
			}
		 }
...
function initialize(){
	...
	var contentString = '<table cellpadding="0" cellspacing="0" border="0" ><tr><td colspan="2"><?php echo $rs_business['name'];?></td></tr><tr><td colspan="2"><img src="<?php echo $ru."media/buisness_images/".$rs_business['locationid']."/logo/thumb/".$rs_business['logo'];?>" title="<?php $rs_business['name'];?>" alt="<?php $rs_business['name'];?>" width="150" hight="10" border="0" /></td></tr><tr><td>Address</td><td ><?php echo $rs_business['address'];?></td></tr><tr><td >Phone No:</td><td ><?php echo $rs_business['phone'];?></td></tr></table>';
	var infowindow = new google.maps.InfoWindow({
...
<div class="header_wrapper">
  <div class="header_wrapper_inner_bar">
    ...
    <div class="learn_more_button_bar">
      <div class="learn_more_button"> <span><a href="<?php echo $ru; ?>learn_more">Learn More</a></span> <img src="<?php echo $ru; ?>images/learn_more_img.png" border="0" /> </div>
    ...
    <div class="facelinks_inner_bar">
      <div class="login_button_outer_bar">
        ...
        <?php
			...
			?>
        <div class="icon_bar"> <a href="http://www.facebook.com/pages/Trades-Tool/236790239750893"><img src="<?php echo $ru; ?>images/facebook_icon.jpg"/></a> <a href="http:///twitter.com/tradestool"><img src="<?php echo $ru; ?>images/twitter_icon.jpg"/></a> 
		
		<a href="<?php echo $ru;?>rss2.php" ><img src="<?php echo $ru; ?>images/rss_icon.jpg"  /></a> 
	<!--<span class='st_email_hcount' displayText='Email'></span>-->
	<a href="mailto:info@tradestool.co.uk" ><img src="<?php echo $ru; ?>images/email_iconb.jpg"/></a>
