...
function business_subscribe(userId){
	$.ajax({
		url:"<?php echo $ru; ?>process/process_subscribe.php?p=sent&userId="+userId+"&bId="+<?php echo $locationid;?>,
...
<?php }else{ 
?><a href="javascript:;">Add To Shortlist</a><?php 
}?></span>
</div>
      <?php 
	  if(!empty($skill_sum)){ 
      ...
      <div class="listing_wrapper_left"> <span class="review">Business Hours</span>
        <div class="business_hour_timing_inner_bar">
		  ...
		  if($rs_business['mon_f']=='' or $rs_business['mon_t']=='')
		  {
		  echo "Close";
		  ...
		  if($rs_business['tue_f']=='' or $rs_business['tue_t']=='')
		  {
		  echo "Close";
		  ...
		  if($rs_business['wed_f']=='' or $rs_business['wed_t']=='')
		  {
		  echo "Close";
		  ...
		  if($rs_business['fri_f']=='' or $rs_business['fri_t']=='')
		  {
		  echo "Close";
		  ...
		  if($rs_business['sat_f']=='' or $rs_business['sat_t']=='')
		  {
		  echo "Close";
		  ...
		  if($rs_business['sun_f']=='' or $rs_business['sun_t']=='')
		  {
		  echo "Close";
	...
	<span class="short_text_span">Your Shortlist<?php /*<span class="edit ">edit</span>*/?></span>
	<?php include("short-list.php"); ?>
	</div>
    <div class="listing_wrapper_left"><?php 
	
	
	$profile_image1  = file_get_contents('ads/profile_image1.html',true);
	echo stripslashes($profile_image1);
	
	$profile_right1  = file_get_contents('ads/profile_right1.html',true);
	echo stripslashes($profile_right1);
	
	$profile_image2  =  file_get_contents('ads/profile_image2.html',true);
	echo stripslashes($profile_image2);
	
	$profile_right2  = file_get_contents('ads/profile_right2.html',true);
...
<script type="text/javascript">
		...
		<?php if(isset($_SESSION['TTLOGINDATA']['USERID'])){
				...
				?>
				jQuery.facebox('You have already review this business.');
				<?php
				}elseif($_SESSION['TTLOGINDATA']['USERID'] == $rs_business['userId']){
				?>
				jQuery.facebox('You are the owner of this profile. You are not allowed to write review on your own profile.');
				<?php 
				}elseif($_SESSION['TTLOGINDATA']['TYPE'] == 'c'){?>
