<?php
	...
	define("ACSQL_DB_USER", 'web92-tradestool');			// <-- mysql db user
	define("ACSQL_DB_PASSWORD", 'qazwsxedc');		// <-- mysql db password
	define("ACSQL_DB_NAME", 'web92-tradestool');			// <-- mysql db name
	define("ACSQL_DB_HOST", 'localhost');						// <-- mysql server host	
	
	define('MESSAGE_DELETE','1'); // How many month till a message is completly <span class="IL_AD" id="IL_AD2">deleted</span>
	
	$ru = "http://".$_SERVER['HTTP_HOST']."/";	        // <-- Website url Path   
	$ruadmin = "http://".$_SERVER['HTTP_HOST']."/cp_tt141/";	        // <-- Website url Path   
	define("ru", $ru);		
	define("DOMAIN", ".TradesTool.co.uk");
	$rootpath = $_SERVER['DOCUMENT_ROOT'] .'/';
	define("ABSPATH", $rootpath);
	$rootadminpath = $_SERVER['DOCUMENT_ROOT'] .'/cp_tt141/';
	*/
	//--------------- local ---------------------------------------------
	
	
	define("ACSQL_DB_USER", 'root');			// <-- mysql db user
	define("ACSQL_DB_PASSWORD", '');		// <-- mysql db password
	define("ACSQL_DB_NAME", 'website');			// <-- mysql db name
	define("ACSQL_DB_HOST", 'localhost');						// <-- mysql server host	
	
	define('MESSAGE_DELETE','1'); // How many month till a message is completly <span class="IL_AD" id="IL_AD2">deleted</span>
	
	$ru = "http://".$_SERVER['HTTP_HOST']."/website/";	        // <-- Website url Path   
	$ruadmin = "http://".$_SERVER['HTTP_HOST']."/website/cp_tt141/";
//	RewriteCond %{HTTP_HOST} !^www\.domain\.com [NC]
//	RewriteCond %{HTTP_HOST} ^((www\.)?([a-z]+)\.)domain\.com [NC]

	
		        // <-- Website url Path   
	define("ru", 'localhost/website');		
	//define("DOMAIN", ".TradesTool.co.uk");
	$rootpath = $_SERVER['DOCUMENT_ROOT'] .'/website/';
	define("ABSPATH", $rootpath);
	
	//echo ABSPATH;exit;//$rootadminpath = $_SERVER['DOCUMENT_ROOT'] .'/tt/cp_tt141/';
...
if(isset($_SESSION['StateAbArray'])){
	$StateAbArray = $_SESSION['StateAbArray'];
	$StateArray =  $_SESSION['StateArray'];
}
else
{
...
function get_DateTimeFormating($dateval){
	...
	$dateparts = explode("-",$date_val);
	
	return $dateparts[1]."-".$dateparts[2]."-".$dateparts[0];
}
function get_DateFormating($date_val){
...
function convert_db_Date($date_val){
...
function businessLogo($Company_row,$ru)
{
	...
	{
		if(!isset($_SESSION['tt_images'])){
		$images = array();
		$sql_images = "SELECT * FROM tt_images";
		$rs_images		= $db->get_results($sql_images, ARRAY_A);
		if($rs_images){
			foreach($rs_images as $rs_row){
				$images[] = $rs_row['photo'].'||'.$rs_row['title'];
			}
			$_SESSION['tt_images']=$images;
		}
	}	
	$arrayMaxIndex = count($_SESSION['tt_images']);
	$randIndex = rand( 0,$arrayMaxIndex -1);
	list($imagename, $imagetitle) =explode("||",$_SESSION['tt_images'][$randIndex]);
	?>
	<img src="<?php echo $ru.'media/carimages/'.$imagename; ?>" title="<?php $imagetitle; ?>" align="right" height="100" width="185" border="0" style="margin-right:20px; margin-top:20px;"  />
	<?php
	}
	else
	{
...
function sendmail($to,$from,$toname,$subject,$msg){
				$msg = nl2br($msg);
				if(trim($from)==''){
					$from = "info@tradestool.co.uk";
				}
				if(trim($subject)==''){
					$subject = "Message from Tradetool.co.uk";
				}
				$headers  = 'MIME-Version: 1.0' . "\r\n";
				$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
				$headers .= 'From: Amerifreight.net <'.$from.'>' . "\r\n";		
...
/*function sendmail($from,$to,$toname,$subject,$msg){
		...
		$headers .= 'From: Trades Tool directory <'.$from.'>' . "\r\n";
		$headers .= "Content-type: text/html\r\n";
		$headers .= "MIME-Version: 1.0\r\n";
		$headers .= "X-Priority: 1\r\n";
		$headers .= "X-MSMail-Priority: High\r\n";
		//echo $to, $subject, $msg, $headers; exit;
		mail($to, $subject, $msg, $headers);
		unset($headers); 
