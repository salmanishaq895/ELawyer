...
$sql = "SELECT * from `tt_citystate` WHERE `city` like '%".$q."%' or `state` like '%".$q."%' order by `city` DESC LIMIT 0,9";
...
if( mysql_num_rows($dataCity ) == 0 ) return;
	...
	{
		array_push($result, array("id"=>$cityRow['city']  , "label"=>$cityRow['city'], "value" => strip_tags($cityRow['city'])));
		if (count($result) > 11)
		break;
	}
